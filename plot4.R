plot4 = function(){
  ...
  d$datetime = weekdays(strptime(as.character(d$Date),"%d/%m/%Y"),abbreviate=TRUE)
  
  ### now plot & save as png
  png(file='plot4.png',width=480, height=480, units="px")
  par(mfrow=c(2,2), cex=.7) # decrese font size for all labels/legend
  # plot 1
  plot(d$Global_active_power, type="l", ann=FALSE, xaxt="n")
  axis(1, at=c(1,which(d$datetime=="Fri")[1],length(d$Global_active_power)+1),
       labels=c("Thu","Fri","Sat"))
  title(ylab='Global Active Power')
  
  # plot 2
  plot(d$Voltage, type="l", ann=FALSE, xaxt="n")
  axis(1, at=c(1,which(d$datetime=="Fri")[1],length(d$Global_active_power)+1),
       labels=c("Thu","Fri","Sat"))
  ...
  # plot 4
  plot(d$Global_reactive_power, type="l", ann=FALSE, xaxt="n")
  axis(1, at=c(1,which(d$datetime=="Fri")[1],length(d$Global_active_power)+1),
       labels=c("Thu","Fri","Sat"))
